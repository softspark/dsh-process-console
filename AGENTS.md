...
- The browser bundle externalizes only `react`. Every harness import is type-only; a value import from a harness package is a bundle-purity error caught by `build-client.mjs`.
- Read data through the public object layer (`ConversationSnapshot`, `SessionListState`) and the framework hooks. No private event registration, no cross-package value imports, no `ctx` in components.
